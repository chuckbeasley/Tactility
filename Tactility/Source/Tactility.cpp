...
namespace app {
    ...
    namespace radar { extern const ::AppManifest manifest; }
...
static void registerInternalApps() {
    ...
    // codes through Nominatim, so there is nothing for it to show without a connection. It brings
    // its own radar screen along, which builds a link rather than doing any fetching of its own.
    app_manager_add(&app::weather::manifest);
    app_manager_add(&app::radar::manifest);
#endif
